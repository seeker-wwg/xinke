...
<div class="my-collection">
    <div class="tit"><span>直播课程</span></div>
    @foreach($livecourse as $v)
    <div class="cont">
        <div class="col-lg-6">
            <div class="item">
                <div class="title">{{$v->name}}</div>
                <div class="time">开始时间: {{date('Y-m-d H:i:s',$v->start_at)}}</div>
                <div class="time">结束时间: {{date('Y-m-d H:i:s',$v->end_at)}}</div>
                <div class="text-center">
                @if(time() >= $v->start_at && time() <= $v->end_at)
                        <a href="{{url('home/livecourse/play',['stream'=>$v->stream_id])}}"
                           target="_blank" class="btn btn-primary">进入直播间</a>
                    @endif
                </div>
            </div>
        </div>
