...
<div class="loginWraper">
  <div id="loginform" class="loginBox">
    @if(!empty($errors->first('errorinfo')))
      <div style="height:20px;background-color: #eea236;">{{$errors->first('errorinfo')}}</div>
    @endif
    @if(!empty($errors->first('mg_name')))
      <div style="height:20px;background-color: #eea236;">{{$errors->first('mg_name')}}</div>
    @endif
    @if(!empty($errors->first('password')))
      <div style="height:20px;background-color: #eea236;">{{$errors->first('password')}}</div>
    @endif
    @if(!empty($errors->first('verify_code')))
      <div style="height:20px;background-color: #eea236;">{{$errors->first('verify_code')}}</div>
    @endif
    ...
    <form class="form form-horizontal" action="{{url('admin/manager/login')}}" method="post">
      ...
      <div class="row cl">
        ...
        <div class="formControls col-xs-8">
          <input id="mg_name" name="mg_name" value="{{old('mg_name')}}" type="text" placeholder="账户" class="input-text size-L">
        </div>
      </div>
      <div class="row cl">
        <label class="form-label col-xs-3"><i class="Hui-iconfont">&#xe60e;</i></label>
...
<div class="footer">Copyright 西藏大学信息科学与技术学院 by H-ui.admin </div>
